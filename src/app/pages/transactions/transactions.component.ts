...
export class TransactionsComponent implements OnInit {
  ...
  constructor(public server: ServerService, private router:Router ,private route: ActivatedRoute ) { 
    let  permitted_routes 
      ...
      this.permissionObj = permitted_routes.find(x=>x.name == 'Transaction Management') 
  }
  ...
  gettransactionList(){
    ...
    this.totalItems = 0;
    let url = apiEndpoint.transaction.getAllTransaction+`?ipAddress=${localStorage.getItem('ip')}&page=${this.currentPage-1}&pageSize=${this.itemPerPage}`
  
    if(this.fromDate && this.toDate) {
      url = url+ `&from=${new Date(this.fromDate).getTime()}&to=${new Date(this.toDate).getTime()}`
    }
    let localdata = localStorage.getItem('data')
    if(localdata == '{"email":"onAlpha@mailinator.com"}'){
      url = url+`&isOneAlpha=true`
    }  
